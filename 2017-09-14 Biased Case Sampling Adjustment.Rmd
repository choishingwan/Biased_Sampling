...
poly.mean <- rnorm(1)
simulated.result <- expand.grid(Sampling=sampling, Prevalence=prevalence, R2=r2, MeanSA=NA, MeanSU=NA, VarSA=NA,VarSU=NA, bVarSA=NA, bVarSU=NA, obsR2=NA, leeR2=NA, pearR2=NA)
for(i in r2)
{
...
sim.result <- melt(simulated.result[,-c(4:9)], id=c("Sampling", "Prevalence","R2"))
ggplot(subset(sim.result, Prevalence<0.2))+geom_abline()+theme_bw()+geom_point(aes(x=R2, y=value, color=Prevalence))+scale_color_distiller(palette = "Spectral")+facet_grid(Sampling~variable)
```
